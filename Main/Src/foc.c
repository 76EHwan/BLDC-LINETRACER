...
void FOC_Reset_State(FOC_Handle_t *hfoc) {
	...
	hfoc->ramped_omega = 0.0f; // [추가] 램프 속도 상태값 초기화
...
//	hfoc->spd_Kd = 0.0f;
	...
	hfoc->spd_Kp = 0.0035f;
	hfoc->spd_Ki = 0.6f;
	hfoc->spd_Kd = 0.000006f;      // 기본은 0에서 시작, 필요 시 튜닝
...
void FOC_Calibrate_Offset(FOC_Handle_t *hfoc) {
	...
	for (int i = 0; i < N; i++) {
		sum_a += (uint16_t) hfoc->ADCx->Instance->JDR1;   // rank1 = 상 A
		sum_c += (uint16_t) hfoc->ADCx->Instance->JDR2;   // rank2 = 상 C
		HAL_Delay(1);
	}
	hfoc->offset_a = (float32_t) sum_a / (float32_t) N;
	hfoc->offset_c = (float32_t) sum_c / (float32_t) N;
}
...
//	float32_t we = hfoc->target_omega;
	...
	float32_t ff_bemf_gain = 1.0f; // TODO: 0.1f로 올려서 테스트
...
//	float32_t v_limit = g_vbus_filt / 2.f;
	float32_t v_limit = g_vbus_filt / M_SQRT3;
	...
	// [8] SVPWM 기반 Duty 연산
	float32_t v_max = fmaxf(v_a, fmaxf(v_b, v_c));
	float32_t v_min = fminf(v_a, fminf(v_b, v_c));
	float32_t v_com = -(v_max + v_min) / 2.0f; // Saddle profile 생성
...
#define MAX_DUTY (PWM_PERIOD * 0.9f)

// @formatter:off
	duty_a = duty_a > MAX_DUTY ? MAX_DUTY : duty_a;
	duty_b = duty_b > MAX_DUTY ? MAX_DUTY : duty_b;
	duty_c = duty_c > MAX_DUTY ? MAX_DUTY : duty_c;
// @formatter:on
...
void FOC_Speed_Loop(FOC_Handle_t *hfoc) {
	...
	// [1] raw CNT 차분 - ENCODER_RESOLUTION 경계(랩어라운드) 보정 포함
	uint16_t cnt = (uint16_t) hfoc->LPTIMx->Instance->CNT; // 0 ~ (ENCODER_RESOLUTION-1)

	int32_t raw_delta = (int32_t) cnt - (int32_t) hfoc->enc_prev_cnt;

	// CNT는 ENCODER_RESOLUTION에서 랩어라운드하므로,
	// ENCODER_RESOLUTION 기준으로 최단 경로(shortest path)를 계산해야 함
	if (raw_delta > (int32_t) (ENCODER_RESOLUTION / 2)) {
		raw_delta -= (int32_t) ENCODER_RESOLUTION;
	} else if (raw_delta < -(int32_t) (ENCODER_RESOLUTION / 2)) {
		raw_delta += (int32_t) ENCODER_RESOLUTION;
	}
	...
	// =========================================================================
	// [추가] 목표 속도 가감속 램프(Soft Start) 프로파일 적용
	// =========================================================================
	float32_t step = hfoc->omega_ramp_rate * SPD_DT; // 1 주기당 변화 가능한 최대 속도량

	if (hfoc->ramped_omega < hfoc->target_omega) {
		hfoc->ramped_omega += step;
		if (hfoc->ramped_omega > hfoc->target_omega) {
			hfoc->ramped_omega = hfoc->target_omega; // 목표값 초과 시 클램핑
		}
	} else if (hfoc->ramped_omega > hfoc->target_omega) {
		hfoc->ramped_omega -= step;
		if (hfoc->ramped_omega < hfoc->target_omega) {
			hfoc->ramped_omega = hfoc->target_omega; // 목표값 미달 시 클램핑
		}
	}

	// [3] 속도 PID - P항, I항 계산 (target_omega 대신 부드럽게 변하는 ramped_omega 사용)
	float32_t err = hfoc->ramped_omega - hfoc->omega_e_meas;
	hfoc->spd_integ += hfoc->spd_Ki * err * SPD_DT;

	// 적분 windup 방지: 적분항을 Iq 한계로 클램핑
	...
	hfoc->spd_prev_meas = hfoc->omega_e_meas;
