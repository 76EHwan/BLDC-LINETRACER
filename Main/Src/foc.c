...
// 2. 모터 제어기 구조체 초기화
void FOC_Init_Motor(FOC_Handle_t *hfoc, TIM_TypeDef *TIMx, ADC_TypeDef *ADCx, LPTIM_TypeDef *LPTIMx) {
    hfoc->TIMx       = TIMx;
    hfoc->ADCx       = ADCx;
    hfoc->LPTIMx     = LPTIMx;
...
void FOC_Calibrate_Encoder_Offset(FOC_Handle_t *hfoc) {
    ...
    while (theta_e_raw >= (float32_t)(2.0 * M_PI)) theta_e_raw -= (float32_t)(2.0 * M_PI);
    while (theta_e_raw < 0.0f)                     theta_e_raw += (float32_t)(2.0 * M_PI);
...
static void FOC_Update_Theta_Encoder(FOC_Handle_t *hfoc) {
    ...
    while (theta_e >= (float32_t)(2.0 * M_PI)) theta_e -= (float32_t)(2.0 * M_PI);
    while (theta_e < 0.0f)                     theta_e += (float32_t)(2.0 * M_PI);
...
void FOC_Execute_Loop(FOC_Handle_t *hfoc, volatile uint16_t *adc_buf) {
    ...
    if (hfoc->V_d > MOTOR_RATED_VOLTAGE)       hfoc->V_d = MOTOR_RATED_VOLTAGE;
    else if (hfoc->V_d < -MOTOR_RATED_VOLTAGE) hfoc->V_d = -MOTOR_RATED_VOLTAGE;

    if (hfoc->V_q > MOTOR_RATED_VOLTAGE)       hfoc->V_q = MOTOR_RATED_VOLTAGE;
    else if (hfoc->V_q < -MOTOR_RATED_VOLTAGE) hfoc->V_q = -MOTOR_RATED_VOLTAGE;
...
void FOC_Speed_Loop(FOC_Handle_t *hfoc) {
    if (hfoc->speed_loop_en == 0) return;
    ...
    if (hfoc->spd_integ >  hfoc->iq_limit) hfoc->spd_integ =  hfoc->iq_limit;
    if (hfoc->spd_integ < -hfoc->iq_limit) hfoc->spd_integ = -hfoc->iq_limit;

    float32_t iq_cmd = hfoc->spd_Kp * err + hfoc->spd_integ;

    // 출력 클램프
    if (iq_cmd >  hfoc->iq_limit) iq_cmd =  hfoc->iq_limit;
    if (iq_cmd < -hfoc->iq_limit) iq_cmd = -hfoc->iq_limit;
...
// 1kHz 타이머 IRQ. 두 모터 속도 루프를 같은 dt로 처리.
