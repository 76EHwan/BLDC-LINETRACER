...
void Ramp_Start() {
	...
	// ★ 영점이동은 4회차에서만 켠다. 여기서 항상 끄고 시작해야
	//   1~3회차 주행이 이전 주행의 오프셋을 물려받지 않는다.
	g_line_offset = 0.f;
	g_line_offset_target = 0.f;
	g_line_offset_rate = 0.f;
...
__STATIC_INLINE void Build_Segment_Kind(uint16_t count) {
	for (uint16_t i = 0; i < count; i++) {
		...
		float_t th45 = next_is_curve ?
				driveData.turn45_len_c_m : driveData.turn45_len_s_m;
		float_t th90 = next_is_curve ?
...
__STATIC_INLINE void Build_Segment_ZeroShift(uint16_t count, uint8_t enable) {
	...
	for (uint16_t i = 0; i < count; i++) {
		...
		uint8_t next_dir =
				(i + 1 < count) ? seg3[i + 1].dir : SEG_DIR_STRAIGHT;
