...
// ============================================================================
// 3회차 주행 함수 모음 (Target Pos 동적 예지 이동 포함)
// ============================================================================
#define TARGET_SHIFT_VAL   (7.f / 15.f)    // 코너 파고들기를 위한 라인 쉬프트 양 (필요시 조절)
#define TARGET_SHIFT_DIST  0.30f  // 마커 전방 15cm부터 부드럽게 Target Pos 변경 시작
...
__STATIC_INLINE void Check_Distance_And_Brake_Third(DriveThirdState_t *state) {
	...
	// 결승선(Pit-in) 정지는 Drive_Stop_At_Distance()가 알아서 처리합니다.
	return;
}

// ★ 3차 주행 전용 Target Position 예지 이동 로직 (수정 완료)
__STATIC_INLINE void Update_Target_Pos_Third(DriveThirdState_t *state) {
	...
	uint8_t current_curve_state = (state->idx > 0) ? seg_plan_3[state->idx - 1].curve_dir : 0;
	// 다가올 다음 마커를 지나친 후의 곡선 상태
	uint8_t next_curve_state = seg_plan_3[state->idx].curve_dir;

	// ★ 부호 수정됨: Left(1) 커브면 라인은 우측(+)에 둬야 차체가 인코스(좌측)를 파고듦
	float current_target = 0.0f;
	if (current_curve_state == 1) current_target = TARGET_SHIFT_VAL;
	else if (current_curve_state == 2) current_target = -TARGET_SHIFT_VAL;

	float next_target = 0.0f;
	if (next_curve_state == 1) next_target = TARGET_SHIFT_VAL;
	else if (next_curve_state == 2) next_target = -TARGET_SHIFT_VAL;

	float traveled = g_odom_distance_m - state->marker_start_dist;
	float remaining = ref_log[state->idx].dist_from_prev_m - traveled;

	// 다음 마커 도달 전 특정 거리(TARGET_SHIFT_DIST)부터 목표점 스무딩
	if (remaining < TARGET_SHIFT_DIST && remaining > 0.0f) {
		// ratio: 0.0(아직 멂) ~ 1.0(마커 도달 직전)
		float ratio = (TARGET_SHIFT_DIST - remaining) / TARGET_SHIFT_DIST;
		// 현재 상태의 목표점에서 다음 상태의 목표점으로 선형 보간 이동
		IR_Sensor.data->target_pos = current_target + (next_target - current_target) * ratio;
	} else {
		// 거리가 충분히 남았을 땐 현재 곡선 상태의 타겟점 유지
...
void Drive_Third(void) {
	...
	// ★ 첫 번째 직선 구간 출발 가속 처리 제거 (기본 속도 유지)
	g_target_base_mps = driveData.base_mps;
